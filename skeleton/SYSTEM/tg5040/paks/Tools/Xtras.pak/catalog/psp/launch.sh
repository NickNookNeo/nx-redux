#!/bin/sh
# ben16w/minui-psp 5.1.0 launch.sh, verbatim except for ONE nx-redux change:
# PAK_DIR is derived from where this script actually lives (made absolute
# below) instead of upstream's hardcoded $SDCARD_PATH/Emus/$PLATFORM/PSP.pak,
# so the pak runs from the flat Emus/PSP.pak install location (nextui's
# getEmuPath() checks that location first - no platform subfolder needed).
# install.sh copies this over the extracted upstream copy; when bumping
# PSP_TAG there, re-sync this file against the new upstream launch.sh.
