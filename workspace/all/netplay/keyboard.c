		...
		for (int col = 0; col < KB_COLS; col++) {
			...
			if (row == KB_ROWS - 1) {
				...
				if (col == 0)
					...
					key_x = start_x + special_key_w + key_spacing;
				else
					key_x = start_x + 2 * (special_key_w + key_spacing);
			} else {
			...
			Uint32 bg_color = selected ? SDL_MapRGB(screen->format, 255, 255, 255) : SDL_MapRGB(screen->format, 60, 60, 60);
